...
export function TimeSeriesChart({
  ...
  if (points.length < 2) {
    return (
      <div className="flex h-[180px] items-center justify-center rounded-sm border border-neutral-800 bg-black text-xs uppercase tracking-wide text-neutral-500">
        Insufficient live history
      </div>
    );
  }
  return (
    <div style={{ height }} className="w-full rounded-sm border border-neutral-800 bg-black p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.025)]">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={points} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
          ...
          />
          <Tooltip
            contentStyle={{ background: "#050505", border: "1px solid #404040", borderRadius: 2, color: "#f5f5f5", fontSize: 12 }}
...
export function CorrelationChart({ data }: { data: Observation[] }) {
  ...
  return (
    <div className="h-[160px] w-full rounded-sm border border-neutral-800 bg-black p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.025)]">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={points} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
          <CartesianGrid stroke="#1f1f1f" vertical={false} />
