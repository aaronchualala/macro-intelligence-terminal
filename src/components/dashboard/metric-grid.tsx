...
function Sparkline({
  ...
  if (points.length < 2) {
    return <div className="h-8 w-32 rounded-sm border border-neutral-900 bg-black" aria-label="Insufficient history" />;
  }
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-8 w-32 overflow-visible rounded-sm bg-black/30" role="img" aria-label="Recent sparkline">
...
export function MetricTable({ metrics }: { metrics: SeriesResult[] }) {
  ...
  return (
    <div className="overflow-hidden rounded-sm border border-neutral-800 bg-black shadow-[0_14px_30px_rgba(0,0,0,0.24)]">
      <div className="terminal-scrollbar overflow-x-auto">
        <table className="min-w-[1180px] w-full border-collapse text-left text-xs">
          <thead className="bg-[#0a0a0a] text-[10px] uppercase tracking-wide text-neutral-500">
            <tr className="[&>th]:border-b [&>th]:border-neutral-800 [&>th]:px-3 [&>th]:py-2.5 [&>th]:font-medium">
          ...
          <tbody className="divide-y divide-neutral-900">
            {metrics.map((metric) => {
              ...
              return (
                <tr key={metric.config.id} className="bg-[#050505] align-middle text-neutral-300 transition-colors hover:bg-[#0d0d0d]">
                  ...
                  <td className="whitespace-nowrap px-3 py-2.5 font-semibold tabular-nums text-neutral-100">{formatMetricNumber(metric)}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 tabular-nums text-neutral-300">{formatDate(metric.stats.latestDate)}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 tabular-nums">{formatMetricNumber(metric, metric.stats.prior)}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 tabular-nums">{formatNumber(metric.stats.mom, unit)}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 tabular-nums">{formatNumber(metric.stats.yoy, unit)}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 tabular-nums">{formatNumber(metric.stats.threeMonthAnnualized, unit)}</td>
                  ...
                  <td className="whitespace-nowrap px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <a
                        ...
                        href={csvHref(metric)}
                        className="rounded-[2px] border border-neutral-800 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-neutral-400 transition hover:border-neutral-600 hover:text-neutral-100"
                      >
                        CSV
                      </a>
                    </div>
